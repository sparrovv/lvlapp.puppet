...
class appserv::app{
  ...
  file { "/home/${settings::app_user}/.${settings::app_name}.env" :
    owner   => "${settings::app_user}",
    group   => "${settings::app_user}",
    mode    => 700,
    content => template('app.env.erb'),
    ensure  => present,
  }
...
alias v=vim
[ -f ~/.${settings::app_name}.env ] && source ~/.${settings::app_name}.env
    "
  }

  logrotate::rule { 'railslogs':
    path          => "/home/${settings::app_user}/${settings::app_name}/shared/log/*.log",
    rotate        => 15,
    rotate_every  => 'week',
    copytruncate  => true,
    compress      => true,
...
class appserv::db{
  class { '::mysql::server':
    ...
    grants                                     => {
      "${settings::app_user}@localhost/${settings::app_name}_production.*" => {
        ensure                                 => 'present',
        options                                => ['GRANT'],
        privileges                             => ['ALL'],
...
class appserv::web{
  ...
  file { "lvlvapp.conf":
    path    => "/etc/nginx/sites-enabled/${settings::app_name}.conf",
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
    content => template('nginx.app.conf'),
